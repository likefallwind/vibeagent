...
from vibeagent.live_dogfood_audit import (
    READ_TOOL_NAMES,
    SIDE_EFFECT_TOOL_NAMES,
    event_command,
    event_succeeded,
    side_effect_paths_within_workspace,
    side_effects_have_prior_approval,
    tool_event_name,
    tool_result_kind,
    transcript_has_no_blocked_commands,
    transcript_has_no_secret_leakage,
)
...
def audit_session_events(root: Path, *, run_id: str) -> list[CheckResult]:
    ...
    first_read_index = next((index for index, event in enumerate(tool_events) if tool_event_name(event) in READ_TOOL_NAMES), None)
    first_side_effect_index = next((index for index, event in enumerate(tool_events) if tool_event_name(event) in SIDE_EFFECT_TOOL_NAMES), None)
    results.append(
        CheckResult(
    ...
    command_events = [event for event in events if event.get("type") == "tool_result" and tool_result_kind(event) in {"run_command", "run_commands"}]
    unittest_events = [event for event in command_events if "unittest" in event_command(event)]
    failed_unittest = [event for event in unittest_events if event_succeeded(event) is False]
    passed_unittest = [event for event in unittest_events if event_succeeded(event) is True]
    results.append(
        CheckResult(
