...
`acceptEdits` -> `ask` plus automatic `Edit` and `NotebookEdit` allow rules,
...
`/check-write-process <id> <text>` to preview writing stdin text to one running background process; quote text with spaces,
`/write-process <id> <text>` to write stdin text to one running background process; quote text with spaces,
