...
| VA1-BACKGROUND | Run and continue an autonomous top-level coding session after the launcher exits | `--background` / `--bg` accepts a persistent one-shot task, writes a private short-lived launch payload, starts the normal CLI in a detached process group, and immediately returns a project-local agent ID. CLI flags and interactive slash commands list sessions, read bounded logs, queue same-session follow-up turns, respawn a running or stopped worker, stop a running process group, and remove supervisor records while preserving the resumable transcript. An atomic FIFO inbox and cross-platform transition lock serialize send against worker exit; immutable worker tokens prevent inherited internal environment from authorizing nested CLI processes; the original API-key-free CLI options and effective worktree root survive respawn. Durable exit markers, PID start-time validation, strict record parsing, owner-only storage, closed stdin, explicit-key rejection, and unchanged approval policy keep detached execution bounded. Interactive attach and approval takeover remain outside the 1.0 contract. |
...
- `tests.test_background_agent_runtime.BackgroundAgentRuntimeTests` and
  ...
  worktree-root continuation, worker-token isolation, failed-spawn rollback,
