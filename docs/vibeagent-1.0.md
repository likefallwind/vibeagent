...
| VA1-BACKGROUND | Run an autonomous top-level coding session after the launcher exits | `--background` / `--bg` accepts a persistent one-shot task, writes a private short-lived launch payload, starts the normal CLI in a detached process group, and immediately returns a project-local agent ID. CLI flags and interactive slash commands list sessions, read bounded logs, stop a running process group, and remove supervisor records while preserving the resumable transcript. Durable exit markers, PID start-time validation, strict record parsing, owner-only storage, closed stdin, explicit-key rejection, and unchanged approval policy keep detached execution bounded. Interactive attach is outside the 1.0 contract. |
...
- `tests.test_background_agent_runtime.BackgroundAgentRuntimeTests` and
  ...
  resumable names, PID-reuse protection, durable completed/failed/stopped
  states, bounded logs, process-group stopping, transcript-preserving removal,
  local CLI routing, machine output, validation, and interactive management.
