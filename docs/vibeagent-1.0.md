...
| VA1-REVIEW | Review changes deeply and block premature completion | `deep_review` runs bounded parallel read-only correctness, security, and test-risk reviewers with root `REVIEW.md` guidance and evidence-oriented reports. Interactive and print-mode `/code-review` share one built-in workflow parser, accept bounded local targets and effort levels, remain read-only by default, and permit edits only with explicit `--fix`; unsupported cloud/comment modes fail before a model request. `final_review`, completion blockers, and suggested verification checks prevent finishing until changed-file review and relevant checks are complete. |
