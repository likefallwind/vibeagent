...
| VA1-PLUGIN | Load and distribute reusable extension bundles without bypassing safety | Repeatable Claude-compatible `--plugin-dir` loads up to 20 validated local non-symlink plugin roots or ZIP archives for one invocation without installation or settings mutation. ZIP plugins support root and single-wrapper layouts, preserve executable modes, use a private content-addressed user cache for cross-process stability, and fail before the model on malformed, oversized, traversal, duplicate, encrypted, link, special-file, or unsafe-cache input. Explicit same-name roots override installed versions, duplicate explicit names fail before the model, and skills, commands, agents, hooks, MCP, LSP, monitors, executables, defaults, user config, interactive catalogs, resume, fork, worktree, background, and subagents share the same resolved root set while events retain only its count. `/plugin` validates, installs, lists, details, enables, disables, and atomically uninstalls project-local plugins; local and public-HTTPS GitHub/Git/JSON marketplaces add, list, inspect, refresh, remove, and install relative or remote `plugin@marketplace` sources; enabled namespaced components flow through their existing parsers, approvals, network checks, and path guards. |
