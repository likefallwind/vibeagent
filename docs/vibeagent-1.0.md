...
| VA1-MONITOR | React to background command output | `Monitor` uses Bash-equivalent approval, delivers complete stdout lines and exit state once as untrusted events during active turns or through idle interactive wakeups, enforces its bounded timeout, supports session-lifetime persistent mode, and can be stopped with `TaskStop` or CLI session exit. |
