...
| VA1-IDE | Work from VS Code with editor-native context and review | The dependency-free extension under `extensions/vscode/` launches VibeAgent in a real integrated terminal, preserving TTY approval behavior. Explicit commands open an interactive session, run a selected-file task, insert a correctly quoted `@path#Lx-Ly` reference, send at most 20 bounded and explicitly untrusted diagnostics, and open the current file against Git `HEAD` in the native diff viewer. Executable settings are machine-scoped, launch arguments bypass shell interpolation, paths stay inside the active workspace or repository, and diagnostics cannot inject control characters or active `@file` references. `scripts/build_vscode_extension.py` creates a deterministic allowlisted VSIX accepted by an isolated VS Code Server installation. |
...
| VA1-REMOTE | Control detached local sessions from a browser | `remote-control` serves a responsive project-scoped control plane for background agents with generated 256-bit bearer authentication, no-store/frame-denied/CSP responses, bounded JSON and text, status/log reads, dispatch and follow-up messages, approval and structured-question responses, and stop/respawn/removal through the existing supervisor locks. It binds loopback by default and requires a caller-supplied regular-file TLS certificate/key pair for non-loopback IPv4. This self-hosted mode does not claim claude.ai account integration, active foreground conversation synchronization, or cross-project aggregation. |
...
- `tests.test_remote_control_server.RemoteControlServerTests` runs the real
  ...
  exercises state, bounded logs, dispatch, reply, approval, question, stop,
...
- `tests.test_vscode_extension.VsCodeExtensionTests` plus the Node tests under
  ...
  references, bounded diagnostic sanitization, command registration, native
  diff routing, deterministic VSIX contents, and JavaScript syntax. A release-
