...
from vibeagent import (
    smart_code_commands,
    smart_code_dependency_commands,
    smart_code_reference_commands,
    smart_code_rename_commands,
    smart_code_symbol_commands,
)
...
class SmartCodeParsingTests(unittest.TestCase):
    ...
    def test_smart_code_symbol_commands_reexports_dependency_commands(self) -> None:
        names = [
            "get_code_deps_report",
            "get_code_deps_text",
        ]
        for name in names:
            with self.subTest(name=name):
                self.assertIs(getattr(smart_code_symbol_commands, name), getattr(smart_code_dependency_commands, name))

    def test_smart_code_symbol_commands_reexports_reference_commands(self) -> None:
        names = [
            "get_code_refs_report",
            "get_code_refs_text",
            "get_code_ref_contexts_report",
        ...
        for name in names:
            with self.subTest(name=name):
                self.assertIs(getattr(smart_code_symbol_commands, name), getattr(smart_code_reference_commands, name))
    ...
    def test_smart_code_dependency_text_helpers_resolve_compatibility_patch_targets(self) -> None:
        deps_report = {"message": "patched deps"}
        root = "/tmp/project"
        with (
            patch("vibeagent.commands.get_code_deps_report", return_value=deps_report) as get_deps,
            ...
            self.assertEqual(smart_code_dependency_commands.get_code_deps_text(root, "web", max_files=7, max_imports=11), "deps text")

        get_deps.assert_called_once_with(root, "web", max_files=7, max_imports=11)
        format_deps.assert_called_once_with(deps_report)

    def test_smart_code_reference_text_helpers_resolve_compatibility_patch_targets(self) -> None:
        refs_report = {"message": "patched refs"}
        contexts_report = {"message": "patched contexts"}
        defs_report = {"message": "patched defs"}
        root = "/tmp/project"
        with (
            ...
            self.assertEqual(smart_code_reference_commands.get_code_refs_text(root, "run web", max_matches=13), "refs text")
            self.assertEqual(
                smart_code_reference_commands.get_code_ref_contexts_text(root, "run web", max_matches=17, context_lines=2, max_bytes_per_context=19),
                "contexts text",
            )
            self.assertEqual(smart_code_reference_commands.get_code_defs_text(root, "run web", max_matches=23, max_lines=29), "defs text")
