...
from vibeagent.file_directory_create_action_executor import execute_directory_create_action
from vibeagent.file_directory_delete_action_executor import execute_directory_delete_action
...
class FileExecutableActionExecutorTests(unittest.TestCase):
    ...
    def test_execute_directory_create_action_returns_none_for_unhandled_actions(self) -> None:
        with tempfile.TemporaryDirectory(prefix="vibeagent-directory-create-executor-") as base:
            workspace = create_run_workspace(base, "test-run")

            self.assertIsNone(execute_directory_create_action(workspace, object()))

    def test_execute_directory_delete_action_returns_none_for_unhandled_actions(self) -> None:
        with tempfile.TemporaryDirectory(prefix="vibeagent-directory-delete-executor-") as base:
            workspace = create_run_workspace(base, "test-run")

            self.assertIsNone(execute_directory_delete_action(workspace, object()))
