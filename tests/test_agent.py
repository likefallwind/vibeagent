...
class AgentTests(unittest.TestCase):
    ...
    def test_run_agent_accepts_todo_write_as_plan_alias(self) -> None:
        with tempfile.TemporaryDirectory(prefix="vibeagent-agent-") as base:
            client = MockClient(
                [
                    [
                    ...
                    [{"type": "text", "text": "Todos are complete."}],
                ]
            )
    ...
    def test_denied_approval_resolution_matches_mcp_tools_server_target(self) -> None:
        observations = [
            ApprovalDeniedObservation(
                kind="approval_denied",
            ...
            types_module.McpToolsObservation(
                kind="mcp_tools",
                ok=True,
                server="docs",
                tools=[],
    ...
    def test_denied_approval_resolution_matches_mcp_call_tool_target(self) -> None:
        observations = [
            ApprovalDeniedObservation(
                kind="approval_denied",
    ...
    def test_denied_approval_resolution_keeps_unrelated_mcp_call_tool_target(self) -> None:
        observations = [
            ApprovalDeniedObservation(
                kind="approval_denied",
                action_type="mcp_call",
            ...
            types_module.McpCallObservation(
                ...
                timeout_ms=10_000,
                error=None,
                message="MCP tool docs/lookup completed.",
            ),
        ]
    ...
    def test_run_agent_continues_after_pending_suggested_check_is_run(self) -> None:
        ...
        self.assertEqual(result.completion_warnings, [])
        self.assertEqual([item.status for item in result.plan], ["completed", "completed"])
        self.assertEqual([item.kind for item in result.observations], ["write_file", "final_review", "run_command", "update_plan", "final_review"])
    ...
    def test_next_action_instruction_reads_summary_structured_completion_next_actions(self) -> None:
        observation = SessionSummaryObservation(
            kind="session_summary",
            run_id="run-1",
            ok=True,
    ...
    def test_approval_preview_summary_ignores_file_preview_after_run_command(self) -> None:
        stale_preview = agent_module.approval_preview_summary(
            types_module.EditFileAction(type="edit_file", path="app.py", old="old", new="new"),
            [
                types_module.CheckEditFileObservation(
    ...
    def test_approval_preview_summary_ignores_file_preview_after_suggested_checks(self) -> None:
        stale_preview = agent_module.approval_preview_summary(
            types_module.EditFileAction(type="edit_file", path="app.py", old="old", new="new"),
            [
                types_module.CheckEditFileObservation(
    ...
    def test_approval_preview_summary_ignores_git_preview_after_run_commands(self) -> None:
        stale_preview = agent_module.approval_preview_summary(
            types_module.GitPushAction(type="git_push"),
            [
                CheckGitPushObservation(
    ...
    def test_approval_preview_summary_ignores_checkpoint_restore_preview_after_session_verification(self) -> None:
        stale_preview = agent_module.approval_preview_summary(
            CheckpointRestoreAction(type="checkpoint_restore", checkpoint_id="ckpt-1"),
            [
                CheckCheckpointRestoreObservation(
                ...
                RunSessionVerificationObservation(
                    ...
                    run_id="run-1",
                    ok=True,
                    selected_commands=[{"command": "python scripts/generate.py", "cwd": "."}],
                    selected_count=1,
                    pending_count=0,
                    failed_count=0,
                    results=[
                    ...
                    stopped_early=False,
                    message="Reran 1 recorded verification command.",
                ),
            ],
        )
