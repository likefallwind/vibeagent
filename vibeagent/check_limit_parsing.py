...
def parse_suggested_checks_limit(argument: str | None = None, default: int = 10) -> int:
    if argument and argument.strip():
        ...
        named_max, positional = parse_suggested_checks_limit_parts(parts)
...
def parse_suggested_checks_limit_parts(parts: list[str]) -> tuple[int | None, list[str]]:
    ...
    while index < len(parts):
        ...
        named_max = parse_named_suggested_checks_limit(raw_value)
    return named_max, positional


def parse_named_suggested_checks_limit(raw_value: str | None) -> int:
