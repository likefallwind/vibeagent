    ...
    return BuiltinModelWorkflow(
        task="\n".join(
            [
                ...
                selection_instruction,
                "Choose a stable lowercase hyphenated skill name run-<app> using at most 64 characters. For a repository-root app, write exactly .claude/skills/<name>/SKILL.md. For a monorepo package, write exactly <package>/.claude/skills/<name>/SKILL.md and use <package>:<name> as the directory-qualified load name. Never write a personal skill or an .agents skill.",
                ...
                "After writing, call project_skills and then skill with the exact generated name or directory-qualified load name. Require the skill to be available from the intended project path with the expected content; treat shadowing, invalid frontmatter, truncation, or load failure as a failed generation and fix it before finishing.",
