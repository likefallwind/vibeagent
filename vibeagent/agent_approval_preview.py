...
def approval_preview_summary(action: object, observations: list[Observation]) -> str | None:
    ...
    for observation in reversed(observations):
        ...
        if preview_search_invalidated(expected_kind, observation_kind):
            return None
        if observation_kind != expected_kind:
            continue
    return None
...
def preview_search_invalidated(expected_kind: str, observation_kind: object) -> bool:
    return expected_kind in GIT_PREVIEW_KINDS and observation_kind in GIT_MUTATION_OBSERVATION_KINDS
