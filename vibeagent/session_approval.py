...
class SessionApprovalHandler:
    ...
    _approved: dict[tuple[str, str, str], ApprovalDecision] = field(default_factory=dict)
...
def approval_cache_key(request: ApprovalRequest) -> tuple[str, str, str] | None:
    if request.action_type in NON_CACHEABLE_APPROVAL_ACTION_TYPES:
        return None
    return request.action_type, request.target, request.preview or ""
