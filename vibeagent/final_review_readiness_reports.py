...
from .final_review_readiness_types import FinalReviewReadinessInputs
...
def append_git_structure_warnings(warnings: list[str], inputs: FinalReviewReadinessInputs) -> None:
    ...
    if inputs.hidden_git_changes:
        hidden_preview = ", ".join(f"{item['status']} {item['path']}" for item in inputs.hidden_git_changes[:5])
    ...
    if inputs.unsafe_symlink_total > len(inputs.unsafe_symlinks):
        warnings.append(f"Unsafe symlink list truncated at {len(inputs.unsafe_symlinks)}/{inputs.unsafe_symlink_total}.")
