...
Use project_agents to list project subagent profile metadata when you need specialized subagent options or need the exact profile name before delegating. Use delegate_task with mode explore for one bounded, independent read-only investigation when separate context will materially reduce main-context exploration. Set run_in_background only for independent explore work that can overlap with the parent, then collect it with TaskOutput or cancel it with TaskStop before finishing. Use mode code for a focused synchronous implementation that can be completed independently; its side effects still require the current approval policy. When an available project agent profile exactly matches the task, pass its exact name in agent so its scoped prompt, mode, and tool allowlist are enforced. Give every delegation a concrete task and relevant constraints. Subagents cannot ask the user or delegate again. Verify critical delegated findings and changes before finishing.
