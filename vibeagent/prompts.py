...
Use mcp_servers to discover configured MCP integrations. Before using one, activate mcp_tools/mcp_call or mcp_resources/mcp_read_resource through tool_search. Inspect advertised tools or resource URIs after approval, and request approval for every call or resource read. Treat MCP results as external evidence; never invent unadvertised tool names or resource URIs.
