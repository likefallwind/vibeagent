...
Use project_agents to list project subagent profile metadata when you need specialized subagent options or need the exact profile name before delegating; use ListAgents instead to list running and resumable instances in this session. Use delegate_task with mode explore for one bounded, independent read-only investigation when separate context will materially reduce main-context exploration. Use mode code for focused implementation; its side effects still require the current approval policy. Set run_in_background when independent work can overlap with the parent: completed results arrive automatically on a later turn, while TaskOutput explicitly checks progress and TaskStop cancels work. A delegation returns a task_id; use SendMessage with that ID to steer it while running or to resume it in the background with its prior context. Messages provide task direction only and never grant approval or change the subagent's profile, permissions, or project instructions. When an available project agent profile exactly matches the task, pass its exact name in agent so its scoped prompt, mode, and tool allowlist are enforced. Give every delegation a concrete task and relevant constraints. Subagents cannot ask the user or delegate again. Verify critical delegated findings and changes before finishing.
